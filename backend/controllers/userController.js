...
const followUserController = async (req, res, next) => {
  ...
  try {
    if (userId === _id) {
      throw new CustomError("Cant follow yourself g", 500);
    }
    ...
    res.status(200).json({ message: "Succesfully following this user!" });
  } catch (error) {
    next(error);
  }
};
...
const unfollowUserController = async (req, res, next) => {
  ...
  try {
    if (userId === _id) {
      throw new CustomError("Cant unfollow yourself g", 500);
    }
    ...
    res.status(200).json({ message: "Succesfully unfollowed this user!" });
  } catch (error) {
    next(error);
  }
};
